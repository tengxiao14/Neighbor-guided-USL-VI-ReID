...
import math
from ChannelAug import ChannelAdap, ChannelAdapGray, ChannelRandomErasing,ChannelExchange,Gray, ChannelExchange_new
...
def main_worker_stage1(args,log_s1_name):
    ...
    logs_dir_root = osp.join(logs_dir_root,str(trial))
    start_time = time.monotonic()

    # cudnn.benchmark = True

    #sys.stdout = Logger(osp.join(args.logs_dir, str(trial)+'log.txt'))
    sys.stdout = Logger(osp.join(logs_dir_root, 'log.txt'))
    print("==========\nArgs:{}\n==========".format(args))
...
##############################
            ...
            mode='all'
            data_path='data/USL-VI-ReID/RegDB/'
            ...
            save_checkpoint({
                ...
                'epoch': epoch + 1,
                'best_mAP': best_mAP,
            }, is_best, fpath=osp.join(logs_dir_root, 'checkpoint.pth.tar'))
